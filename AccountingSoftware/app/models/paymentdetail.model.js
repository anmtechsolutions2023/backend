...
exports.deleteById = async (id, tenantId, username) => {
  return await deleteById.deleteById(
    ...
    tenantId,
    username,
    moduleScripts.paymentdetail.delete,
    ...
    switch (queryParams.QueryParamName) {
      ...
      default: {
        logger.loggerHelper(
          ...
          i18n.__('messages.modules.paymentdetail.queryParamNotSupported')
        )
...
exports.update = async (updatedReq, username) => {
  try {
    const query = moduleScripts.paymentdetail.update
...
exports.create = async (req, username) => {
  try {
    const query = moduleScripts.paymentdetail.create
    const reqId = uuidv4()
    ...
    logger.loggerHelper(
      ...
      i18n.__('messages.logger.errorCreatedById', {
        name: `${req.AccountTypeBaseId}-${req.TransactionDetailLogId}`,
        code: err.code,
        message: err,
      })
    )
