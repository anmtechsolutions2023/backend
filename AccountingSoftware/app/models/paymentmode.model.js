...
exports.deleteById = async (id, tenantId, username) => {
  return await deleteById.deleteById(
    ...
    tenantId,
    username,
    moduleScripts.paymentmode.delete,
    moduleNames.paymentmode.db.delete
  )
}
...
exports.getAll = async (tenantId, username) => {
  return await getAllModels.getAll(
    tenantId,
    username,
    moduleScripts.paymentmode.fetchAll,
    moduleNames.paymentmode.db.fetchAll
  )
}
...
exports.searchByParam = async (tenantId, username, queryParams) => {
  try {
    ...
    switch (queryParams.QueryParamName) {
      case 'Type':
        query = moduleScripts.paymentmode.fetchAll + ' AND Type = ?'
        break
      default: {
        logger.loggerHelper(
          tenantId,
          username,
          moduleNames.paymentmode.db.searchbyname,
          logger.logType.error,
          i18n.__('messages.modules.paymentmode.queryParamNotSupported')
        )
...
exports.update = async (updatedReq, username) => {
  try {
    const query = moduleScripts.paymentmode.update
    ...
    logger.loggerHelper(
      updatedReq.TenantId,
      username,
      moduleNames.paymentmode.db.update,
      logger.logType.debug,
...
exports.findById = async (id, tenantId, username, callerModule) => {
  return await getFindById.findById(
    ...
    tenantId,
    username,
    moduleScripts.paymentmode.fetchById,
    `${callerModule}--${moduleNames.paymentmode.db.fetchById}`
  )
}

exports.create = async (req, username) => {
  try {
    const query = moduleScripts.paymentmode.create
    const reqId = uuidv4()
    ...
    logger.loggerHelper(
      req.TenantId,
      username,
      moduleNames.paymentmode.db.create,
      logger.logType.debug,
